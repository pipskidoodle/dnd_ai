...
describe('extractJSONObjectResponse', () => {
  ...
  "Ultimately, Belsky thinks that generative AI will be used for practical business use cases that reduce manual work and speed up processes, such as enabling content velocity and personalization": true
    ...
    it('Should return null when there are no JSON array', () => {
        expect((0, extracter_1.extractJSONObjectResponse)(`hello world! [ "val 1", "val 2" ] end`)).toBe(undefined);
    });
    it('Should extract a JSON object response with no chars in front or behind', () => {
        expect((0, extracter_1.extractJSONObjectResponse)(testJSONObject)).toEqual(testJSONObject);
    });
    it('Should extract when there are chars only in front', () => {
        expect((0, extracter_1.extractJSONObjectResponse)(`text in front ${testJSONObject}`)).toEqual(testJSONObject);
    });
    it('Should extract when there are chars both in front and behind', () => {
        expect((0, extracter_1.extractJSONObjectResponse)(`text in front ${testJSONObject} text behind`)).toEqual(testJSONObject);
    });
    it('Should extract where there are only chars behind', () => {
        expect((0, extracter_1.extractJSONObjectResponse)(`${testJSONObject}more text behind`)).toEqual(testJSONObject);
    });
});
describe('extratJSONArrayResponse', () => {
    const testJSONArray = `["statement 1", "statement 2", "statement 3"]`;
    it('Should return null when there are no JSON array', () => {
        expect((0, extracter_1.extractJSONArrayResponse)(`hello world! { "key": "value" } end`)).toBe(undefined);
    });
    it('Should extract a JSON array response with no chars in front or behind', () => {
        expect((0, extracter_1.extractJSONArrayResponse)(testJSONArray)).toEqual(testJSONArray);
    });
    it('Should extract when there are chars both in front and behind', () => {
        expect((0, extracter_1.extractJSONArrayResponse)(`text in front ${testJSONArray} text behind`)).toEqual(testJSONArray);
    });
});
describe('extractBulletPointsResponse', () => {
  ...
  `;
    it('Should extract bullet point string into an array', () => {
        expect((0, extracter_1.extractBulletPointsResponse)(testString)).toEqual([
            'bullet point 1',
            'bullet point 2',
            'bullet point 3 - with another dash in the middle.',
        ]);
